...
public static partial class ResultExtensions
{
    ...
    /// <returns>A successful Result when no exception is thrown; otherwise a failed Result.</returns>
    public static ValueTask<Result> TryAsync(Func<ValueTask> action, Func<Exception, IError> errorHandler)
    ...
    /// <returns>A successful Result when no exception is thrown; otherwise a failed Result.</returns>
    public static async ValueTask<Result> TryAsync(Func<ValueTask> action, Func<Exception, IEnumerable<IError>> errorHandler)
    {
        ArgumentNullException.ThrowIfNull(action);
    ...
    /// <param name="errorHandler">Exception-to-error mapper. Defaults to exception message.</param>
    /// <returns>A successful Result containing the function value when no exception is thrown; otherwise a failed Result.</returns>
    public static ValueTask<Result<TValue>> TryAsync<TValue>(Func<ValueTask<TValue>> func, Func<Exception, IError> errorHandler)
    ...
    /// <param name="errorHandler">Exception-to-errors mapper. Defaults to exception message.</param>
    /// <returns>A successful Result containing the function value when no exception is thrown; otherwise a failed Result.</returns>
    public static async ValueTask<Result<TValue>> TryAsync<TValue>(Func<ValueTask<TValue>> func, Func<Exception, IEnumerable<IError>> errorHandler)
    {
        ArgumentNullException.ThrowIfNull(func);
        errorHandler ??= DefaultErrorsHandler;
